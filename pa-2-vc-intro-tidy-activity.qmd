...
**NAMES:** Dalton Garcia and Trevor Edgull
...
# making one long vector out of all our symbols names my_symbols
...
```{r}
#| label: below-17
#confirming 352 numbers are below 17
...
```{r}
#| label: matrix-5-column
# Turned our vector of numbers into a matrix with 5 columns.
...
```{r}
#| label: separate_secret_numbers
#create a vector of all the even numbers between 1 and 382.
...
```{r}
#| label: subtract-from-vectors
#modifying vectors by subtracting, multiplying, and turning matrix into vector
...
```{r}
#| label: verify-indices500-below100
...
```{r}
#| label: square-root-and-round
#taking the square root of numbers in indicies 38 to 465 and rounding the values.
